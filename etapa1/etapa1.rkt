...
(define (longest-common-prefix w1 w2)
  ...
  (vreau-acc w1 w2 '()))
;(longest-common-prefix (string->list "a") (string->list "ana"))
...
(define (longest-common-prefix-of-list words) ;ne folosim de longest common prefix anterior 
  (if (null? (cdr words))
      (car words)
  (let*
      ((prefix-curent (car(longest-common-prefix (car words) (cadr words)))) )
         ...
         (begin ; ok aparent nu ma lasa cu define nici asa folosesc let 
            ...
            (if (equal? prefixc pattern)
                ...
                (if (equal? prefixc (get-branch-label branch))
                    ...
                    (list #f (car(longest-common-prefix (get-branch-label branch) pattern)))))))))) ;nu l-am gasit complet ci doar partial
