...
; st-empty? : ST -> Bool
(define (st-empty? st)
  (if (stream-empty? st) #t
      #f))

; operatorul care extrage prima ramură a unui ST
; first-branch : ST -> (Label, ST)
;branch e pereche de label si subtree care substree e flux
...
(define (get-ch-branch st ch)
  (cond 
    ((st-empty? st) #f)
    ((char=? (car(get-branch-label (first-branch st))) ch) (first-branch st)) ;daca nu ii pun caar o ia razna la $
